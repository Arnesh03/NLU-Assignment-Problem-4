...
\begin{myabstract}
This report details the development and evaluation of a text classification system designed to distinguish between documents from the \textbf{Sports} and \textbf{Politics} domains. Using the 20 Newsgroups dataset, we implemented and compared four machine learning models: Multinomial Naive Bayes, Logistic Regression, Linear Support Vector Machine, and Random Forest. Features were represented using TF-IDF vectorization with unigrams and bigrams. Our results show that even simple linear models achieve high accuracy (above 92\%), with Naive Bayes performing the best in various trials.
\end{myabstract}
...
\begin{itemize}
    ...
    \item To compare the performance of four different machine learning algorithms.
...
\begin{enumerate}
    ...
    \item \textbf{Random Forest (RF)}: An ensemble learning method that constructs multiple decision trees during training and outputs the mode of the classes for classification.
\end{enumerate}
...
\begin{table}[H]
    ...
    \begin{tabular}{lcccc}
        ...
        Linear SVM & 95.1\% & 0.94 & 0.98 & 0.96 \\
        Random Forest & 92.7\% & 0.95 & 0.93 & 0.94 \\
        \bottomrule
    \end{tabular}
\end{table}
...
\subsection{Analysis}
Naive Bayes achieved the best overall performance, likely due to the highly distinct vocabularies of the two domains. Logistic Regression and SVM followed closely. Random Forest, while still very accurate at 92.7\%, slightly lagged behind the linear models. This suggests that the relationship between word frequencies and classes is largely linear for this specific task, and the additional complexity of an ensemble forest doesn't provide a boost in this binary classification scenario.
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{Confusion Matrices for all four machine learning models.}
\end{figure}
